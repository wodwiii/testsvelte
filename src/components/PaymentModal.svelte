...
// @ts-nocheck
    ...
    import { createCustomer,createSubscription,createPaymentMethod,attachPaymentMethod } from '$lib/api-call';
...
const handlePaymentSubmit = async () => {
    try {
        ...
        const subscriptionResponse = await createSubscription(customerResponse.data.id, 'plan_zwSb1Bkd2Jaize3weMAKxeRA');
        ...
        const attachPaymentMethodResponse = await attachPaymentMethod(paymentIntentId, paymentMethodResponse.data.id);
...
<Modal show={showPaymentModal} onClose={closeModal} title="Enter Payment Details">
    <form on:submit|preventDefault={handlePaymentSubmit}>
        ...
        <div class="mb-4">
            <label class="block text-gray-700">First Name 
                <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={firstName} required />
            </label>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700">Last Name 
        ...
        <div class="mb-4">
            <label class="block text-gray-700">Phone Number 
                <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={phoneNumber} required />
            </label>
        </div>

        <!-- Collapsible Billing Address -->
        <div class="mb-4">
            <button type="button" on:click={() => showBillingAddress = !showBillingAddress} class="text-[#fe0000] hover:underline">
                {showBillingAddress ? '' : '+'} Billing Address
            </button>
            {#if showBillingAddress}
                <div class="mt-4 space-y-2">
                    <label class="block text-gray-700">Billing Address Line 1 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={billingAddress1} required />
                    </label>
                    <label class="block text-gray-700">Billing Address Line 2 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={billingAddress2} />
                    </label>
                    <label class="block text-gray-700">City 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={city} required />
                    </label>
                    <label class="block text-gray-700">State 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={state} required />
                    </label>
                    <label class="block text-gray-700">Postal Code 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={postalCode} required />
                    </label>
                    <label class="block text-gray-700">Country 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={country} required />
                    </label>
                </div>
            {/if}
        </div>
        ...
        <div class="mb-4">
            <button type="button" on:click={() => showCardInformation = !showCardInformation} class="text-[#fe0000] hover:underline">
                {showCardInformation ? '' : '+'} Card Information
            </button>
            {#if showCardInformation}
                <div class="mt-4 space-y-2">
                    <label class="block text-gray-700">Card Number 
                        <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={cardNumber} required />
                    </label>
                    <div class="flex space-x-4">
                        <div class="w-1/2">
                            <label class="block text-gray-700">Expiration Month 
                                <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={expirationMonth} required />
                            </label>
                        </div>
                        <div class="w-1/2">
                            <label class="block text-gray-700">Expiration Year 
                                <input type="text" class="mt-1 p-2 border rounded w-full" bind:value={expirationYear} required />
                            </label>
                        </div>
                    </div>
                    <label class="block text-gray-700">CVC 
